  ...
  <section class="chat">
    ...
    <div class="container">
      <div class="chat-wrapper">
        <div style="display: none" class="chat-left" :class="{ active: isActive }">
          <label class="chat-label">
            <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              />
              <path
                d="M21.0004 21.2813L16.6504 16.9313"
                stroke="#C6C6C6"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
          ...
          <div
            ...
            :key="i"
            @click=";(online = item.lastMessage), (name = item.name)"
            class="chat-block"
          >
        ...
        <div class="chat-right" :class="{ active: isActive }">
          <div class="chat-head">
          ...
          <div class="chat-content">
            <div
              ...
              :key="i"
              class="chat-item"
              :class="{ my: item.result.sender == store.state.userInfo.id }"
            >
              {{ item.result.text }}
          ...
          <label class="chat-label">
            ...
            <input class="chat-another" type="text" placeholder="Введите сообщение" v-model="personText" />
...
const getActiveuser = async () => {
  const res = await fetch('http://89.208.106.189/api/v1/chat/' + JSON.parse(localStorage.getItem('chatId')))
  const data = await res.json()
  messageList.value = data.messages
  const dateTimeString = JSON.parse(localStorage.getItem('chatInfo')).created_date
  const dateTime = new Date(dateTimeString)
  const hours = dateTime.getHours()
  const minutes = dateTime.getMinutes()
  online.value = `${hours}:${minutes}`
  const authorRes = await fetch("http://89.208.106.189/api/v1/author/" + router.params.id);
  const authorData = await authorRes.json()
  console.log(authorData);
  name.value = authorData.first_name + ' ' + authorData.last_name
}
...
const ws = new WebSocket('ws://89.208.106.189/ws/socket-server/chat/' + JSON.parse(localStorage.getItem('chatId')))
...
ws.onmessage = event => {
  ...
  console.log('Получено сообщение:', receivedData)

  const message = {
    type: 'chat_message',
    result: {
      text: receivedData.result.text,
      sender: receivedData.result.sender,
    },
  }
